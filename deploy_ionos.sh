...
"$PY" - "$ZIP" <<'PYEOF'
import sys, zipfile
# Two deliberate exceptions under data/ (2026-08-23), both generated by THIS build rather than being host
# runtime state, and both meaningless unless overwritten with the current release's copy:
#   version_history.json - the release manifest the Version history tab falls back to, because the
#                          deployed package has no .git for its usual `git log`.
#   build_id.json        - the fingerprint /api/build reports, which is how the deploy proves the resident
#                          Flask module is running this release and not a previously-loaded one.
# Everything else under data/ stays protected: credentials, the IG close audit trail, runtime caches.
ALLOWED = {"data/version_history.json", "data/build_id.json"}
