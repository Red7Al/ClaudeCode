...
name: ah-hvf-orders
description: >
  The daily HVF ORDERS publication — the actionable, tradeable HVF setups posted to
...
# AH HVF Orders — the daily candidate-orders publication

A once-daily Slack publication of the **actionable** HVF setups — the ones at/near entry that
clear the tradeability bar — framed as **candidate orders** ("not yet placed"). It is the
"result of running the HVF analysis to slack-orders each day" (user 2026-06-19).
...
- Posts **only TRADEABLE** setups (READY/TRIGGERED with R:R ≥ `HVF_MIN_RR`, currently 3:1).
...
|---|---|---|---|
| **HVF Orders** (this) | fresh TRADEABLE HVF setups — *candidate* orders, not yet placed | #arw-claude-orders | `run_hvf_orders.py` |
...
| HVF Daily Report (`ah-hvf-report`) | TRADEABLE **+ DEVELOPING**, full analytical view | #arw-claude-signals | `run_hvf_report.py` |

## Schedule

- `trading-hvf-orders.yml` workflow, cron job **"HVF Orders" `0 6 * * 1-6`** (06:00 UTC Mon–Sat),
...
Same five-rule Hunt Volatility Funnel method as the rest of the system (see `ah-hvf-analysis` /
