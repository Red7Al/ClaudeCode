...
Every emitted squeeze satisfies **all five**. The per-instrument status of each is shown in the
dossier (`instrument_dossier._hvf_rules_block`).

| # | Rule | Threshold (source of truth) |
|---|------|------------------------------|
| 1 | **Prior trend confirmed** | uptrend → bullish squeeze, downtrend → bearish (not sideways) |
| 2 | **Lower highs** | `H1 > H2 > H3` (the ceiling is falling) |
| 3 | **Higher lows** | `L1 < L2 < L3` (the floor is rising) |
| 4 | **Squeeze converges** | width contracts **≥ 30%** from `H1−L1` to `H3−L3` (i.e. convergence ratio `< 0.70`) |
...
(`config.MIN_RISK_REWARD`, currently **3.0**). Below it the squeeze is valid but flagged
...
- **Entry** = `H3` for a bullish squeeze (a break of the ceiling), `L3` for a bearish squeeze.
...
The raw squeeze projects a target of roughly one pattern-height from the breakout. The system then
...
squeeze's `H2` and measures AMP1 from that exhaustion point to the first natural-support pullback.

- Only the **target amplitude** is re-anchored; entry and stop stay at the squeeze's third pivots.
...
(`MIN_PUBLISH_QUALITY`, currently **25**): setups below it are not published to X.

> This floor was **70 until 2026-06-22**, when the user lowered it to 25 so that middling-quality clean
> funnels (e.g. USDJPY at quality 46) promote to tradeable/publishable rather than sitting in developing —
> the structural rules already gate hard on strict swings, tightness ≤ 35% and R:R ≥ 3, so quality here is
> a softer ranking floor. `config.py` is the source of truth; this document said 70 for two months after
> the change, which is why the value is now stated with its history rather than on its own.

---
*Visuals regenerated by `docs/_gen_doc_visuals.py`. Last updated 2026-08-25 (publication floor corrected
from a stale 70 to the live 25; thresholds re-checked against `config.py` and `price_action.py`).*
